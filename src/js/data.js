...
export const data = {
  projects: [
    ...
    },
    {
      ...
      description:
        "Seller's Place is a SaaS application that helps event vendors and promoters to connect, but also it is also my first real application project I've worked on. As a full-stack developer for Seller's Place, my fingerprints are all over this project as I helped build many of the frontend web components, as well as helped create API's and stored procedures in the backend. As far as technologies, we worked in React.js in the frontend, Node.js and Microsoft SQL Server in the backend. As seen in the demo, some of the features I worked on in this project were the 'Invite Email' feature, the online payment service, transaction history log, product type management system, and the venue details page. The 'Invite Email' service uses SendGrid API to allow event promoters to reach out to specific vendors and send an invitation email to them directly. The online payment service utilizes Node Stripe API to ensure secure transactions. The transaction history log uses data regarding a specific user's payment history pulled from SQL database to create a paginated visual log of their transactions. Additionally, I Designed and developed the stored procedures, Node.js endpoints, and front-end services (using Axios) to receive this data. The Product Type Management System, was created for users to manage their product types on a single page. Editing for this page was done by inline editing component custom designed for convenience and better UX. Lastly, the Venue Details page was built to display a targeted venue. It uses the Google Maps API and react-geocode to render a map of the venue for user to view. Additionally, created stored procedures in SQL to grab data for the six most recent events of said venue."
    },
    {
      title: "McMacro",
      name: "mcmacro",
      ...
      description:
        "This is my webscraping project I built for days when I was too lazy to cook, but still want to reach my macronutrient goals for the day. I am using React.js as the frontend framework, an accordion component from Material-UI, and Chart.js to render the beautiful pie chart. For the data, I am using Cheerio.js in Node.js to scrape a very convinient website, fastfoodmacros.com. It's no surpise here that most of my application magic occurs in the Node.js/Express.js environemnt as I use user input data to filter through the scraped data, and return a list of restaurants and items that match the given criteria. I look forward to booting this baby up next January for my annual fitness resolution that lasts a few months.",
